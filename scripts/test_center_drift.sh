...
OUT="${1:-$PROJECT/output/truck_center_drift_lr1e4_$(date +%Y%m%d_%H%M%S)}"
...
echo "GPU: $CUDA_VISIBLE_DEVICES; random start; self-only decoder; fixed LR 1e-4; 5000 center steps"
