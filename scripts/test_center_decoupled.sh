...
OUT="${1:-$PROJECT/output/truck_center_decoupled_$(date +%Y%m%d_%H%M%S)}"
...
echo '5000 center steps; base LR 1e-4; same-batch backtracking 1, 1/2, 1/4, 1/8, 1/16'
exec "$PYTHON_BIN" -u -m gaussian_jscc train-center-attributes \
  ...
  --center-position-layout centroid_residual --center-step-guard --center-max-backtracks 4 \
