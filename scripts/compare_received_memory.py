"""Larger matched CPU experiment: trunk vs per-layer received-memory reads.
...
def reference_case(root, seed, protocol):
    """Reuse only a completed, protocol-matched NONE baseline; never a candidate."""
    root=Path(root)
    previous=json.loads((root/'protocol.json').read_text(encoding='utf-8'))
    for key in ('ply_sha256','source_gaussians','steps','every','save_every','blocks',
                'blocks_per_batch','indices','splits','codec_config','threads'):
        if previous[key]!=json.loads(json.dumps(protocol[key])):
            raise ValueError(f'reference protocol mismatch: {key}')
    if seed not in previous['seeds'] or 'none' not in previous['modes']:
        raise ValueError('reference lacks baseline seed')
    folder=root/f'none_seed{seed}'
    if not folder.is_dir():folder=root/f'seed{seed}'/f'none_seed{seed}'
    rows=[json.loads(s) for s in (folder/'validation.jsonl').read_text().splitlines()]
    if rows[-1]['step']!=protocol['steps']:
        raise ValueError('incomplete reference baseline')
    return folder
...
def run(args):
    ...
    if args.reference_root:
        for seed in args.seeds:
            reference_case(args.reference_root,seed,protocol)
    ...
    for seed in args.seeds:
        for mode in args.modes:
            if mode=='none' and args.reference_root:
                source=reference_case(args.reference_root,seed,protocol)
                folder=out/f'none_seed{seed}'
                shutil.copytree(source,folder)
                (folder/'reference.json').write_text(json.dumps(dict(
                    source=str(source.resolve()),
                    validation_sha256=file_hash(source/'validation.jsonl'),
                    protocol_sha256=file_hash(Path(args.reference_root)/'protocol.json')),
                    indent=2),encoding='utf-8')
                print(f'Reused matched baseline seed={seed}: {source}',flush=True)
                continue
            torch.manual_seed(seed)
            config = cfg.to_dict(); config['decoder_memory'] = mode
...
def build_parser():
    ...
    parser.add_argument('--reference-root',help='optional completed matching experiment supplying NONE baselines only')
    parser.add_argument('--modes',nargs='+',choices=['none','received'],default=['none','received'])
    return parser
...
def run_parallel(args):
    ...
    def worker(seed):
        ...
        if args.reference_root:cmd.extend(['--reference-root',args.reference_root])
