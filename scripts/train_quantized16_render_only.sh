#!/usr/bin/env bash
# Fixed 16-bit coordinates, losslessly delta/shuffle/zlib coded; attributes JSCC.
set -euo pipefail
PROJECT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$PROJECT"
: "${CUDA_VISIBLE_DEVICES:?Set CUDA_VISIBLE_DEVICES to an available GPU}"
PYTHON_BIN="${PYTHON_BIN:-python}"
OUT="${1:-$PROJECT/output/truck_quantized16_zlib_render_$(date +%Y%m%d_%H%M%S)}"
...
echo 'Compression size is measured once per retention mask; first validation may pause for CPU compression.'
