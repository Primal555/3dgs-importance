...
def plot_run(out):
    ...
    if guarded:
        ...
        groups = {branch: [a['branches'][branch] for a in g] for branch in ('common', 'local')} if 'branches' in g[0] else {'joint': g}
        for label, entries in groups.items():
            axes[0].plot(x, [a['scale'] for a in entries], linewidth=.6, label=label)
        axes[0].legend()
        ...
        for label, entries in groups.items():
            ...
            axes[2].plot(x, fractions, label=label)
        axes[2].legend()
        ...
        if 'branches' in g[0]:
            ...
            for label, entries in groups.items():
                restarts, fractions = 0, []
                for i, entry in enumerate(entries):
                    restarts += entry['momentum_restarted']
                    fractions.append(restarts/(i+1))
                axes[0].plot(x, fractions, label=label)
                axes[1].plot(x, [a['attempts'][0]['directional_derivative'] for a in entries],
                             linewidth=.6, label=label)
            axes[0].set_title('Cumulative first-moment restart fraction')
            axes[1].set_title('Original Adam directional derivative (<0: downhill)')
            ...
            for axis in axes:
                ...
                axis.legend()
            fig.tight_layout()
            fig.savefig(charts/'center_momentum_restarts.png', dpi=150)
            plt.close(fig)
